...
dd if=/dev/zero of=${IMAGE_NAME} bs=10M count=530 || assert
(
    echo "o"
    echo "n"
    echo "p"
...
cp ${SCRIPTROOT}/ArchLinuxARM-rpi-aarch64-latest.tar.gz . || wget -c http://os.archlinuxarm.org/os/ArchLinuxARM-rpi-aarch64-latest.tar.gz || assert
...
arch-chroot /mnt/ /usr/bin/bash -c 'pacman --disable-download-timeout --noconfirm -Sy arch-install-scripts pacman-contrib wget' || assert
...
## Install packages
arch-chroot /mnt/ /usr/bin/bash -c 'sed -i "s/#ParallelDownloads = 5/ParallelDownloads = 20/g" /etc/pacman.conf' || assert
arch-chroot /mnt/ /usr/bin/bash -c 'pacman --disable-download-timeout --noconfirm -Suy \
                                            linux-rpi \
